...
void loop() {
  ...
  if (I_F >= I_target) { // reached target for the first time
  ...
  else if(readMux(3) > 650 || readMux(4) > 650 || readMux(5) > 650 || (readMux(3) > 120 && readMux(4) > 120) || (readMux(4) > 120 && readMux(5) > 150)){ //Obstacle at front
    count = count + 1;
    ...
    TrSum = TrSum + t4 - t3;
      ...
      do{
        ...
        motor2.drive(Vl+3);
        t6 = millis();
      }while(t6 < (t5 + dT) && analogRead(F_LDRpin) < I_target && readMux(3) < 650 && readMux(4) < 650 && readMux(5) < 650);
  }
}
