...
// Input parameters
double V = 10;         // [V] in cm/s , it defines the range [-V, V]_uniform
double v_active = 1;   // [v_a] in cm/s, active velocity, make it zero for Brownian motion
int    epsilon = 100;  // [\epsilon] in ms
...
void loop() {
  ...
  // Mimic ABP with translation noise
  else{
    // Generate vl and vr
    Vr = random(-100,101)/(100.00/V);
    Vl = random(-100,101)/(100.00/V);
    Vr = Vr + v_active;                // In cm/s
    Vl = Vl + v_active;                // In cm/s
